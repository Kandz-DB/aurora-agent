...
app.delete('/api/projects/:id', async (req, res) => {
  try {
    ...
    console.log(`[Delete] Removing project: ${exists.clientName} (${id})`);
    await db.deleteProject(id);
    const after = await db.getProjects();
    const stillExists = after.find(p => p.id === id);
    if (stillExists) {
      console.error(`[Delete] FAILED — project still in storage after delete`);
      return res.status(500).json({ error: 'Delete failed — project still exists in storage' });
    }
    console.log(`[Delete] Success — ${before.length} → ${after.length} projects`);
